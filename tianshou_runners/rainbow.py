""" Wrapper to use Rainbow DQN Policy in bsuite experiment.
    Code adapted from: https://github.com/thu-ml/tianshou/blob/master/tianshou/policy/modelfree/rainbow.py
    Implementation of Rainbow Deep Q-Network. arXiv:1710.02298.
"""
...
class default_agent(base.Agent):
  def __init__(self, obs_spec, act_spec, buffer_size=128):
    ...
    self.net = ts.utils.net.common.Net(self.obs_shape, self.act_n, hidden_sizes=[64,64], num_atoms=51, device=device)
    self.optim = torch.optim.Adam(self.net.parameters(), lr=0.0003)
    self.agent = ts.policy.RainbowPolicy(self.net, self.optim, num_atoms=51).to(device)
