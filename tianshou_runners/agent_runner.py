...
# ============================================================================
# Code adapted from: https://github.com/deepmind/bsuite/blob/main/bsuite/baselines/tf/dqn/run.py
# Changes: using tianshou agents
#          added timing and model-saving functionality
#          allows editing of num_processes
#          garbage collection
# ============================================================================
""" Run a tianshou agent instance (using Pytorch) on a bsuite experiment."""
...
flags.DEFINE_string('agent', 'c51', 'which agent to use')
...
def run(bsuite_id: str) -> str:
  """Runs a tianshou agent on a given bsuite environment, logging to CSV."""
  fpath = f"{FLAGS.save_path}/models/{bsuite_id.replace('/', '_')}.pth"
  ...
  exec(f'from {FLAGS.agent} import default_agent', globals())
  def get_agent(): return default_agent(env.observation_spec(), env.action_spec(), 1e4)
  agent = get_agent()
  ...
  torch.save(agent.agent.state_dict(), fpath + f"{bsuite_id.replace('/', '_')}.pth")
